 ...
 *
 * Usage:
 *   node scripts/esbuild.mjs <distDir...> [--obfuscate] [--bundle-deps] [--declaration]
 *
 * Requirements:
...
const shouldBundleDeclaration = argv.includes('--declaration');
...
if (dirs.length === 0) {
  throw new Error('Usage: node scripts/esbuild.mjs <distDir...> [--obfuscate] [--bundle-deps] [--declaration] (expects module.js in each distDir)');
}
...
const bundleDir = async (distDir) => {
  ...
  const keepDtsAbs = new Set(keepJs.map((fileName) => path.join(distDir, fileName.replace(/\.js$/, '.d.ts'))));
  ...
  if (shouldBundleDeclaration) {
    for (const fileName of keepJs) {
      ...
      const bundle = await rollup({
        input: dtsDest,
        plugins: [dts()],
        external: [/^matterbridge/, /^moment/, /^node-ansi-logger/],
      });
  ...
  // Remove generated JS artifacts, keep only the entry-point type declaration.
  ...
  for (const filePath of allFiles) {
    ...
    if (filePath.endsWith('.d.ts') && !keepDtsAbs.has(filePath)) {
      await rm(filePath, { recursive: true, force: true });
      removedEntries.push(path.relative(distDir, filePath));
      continue;
    }
