...
vi.mock('../src/helpers.js', async (importOriginal) => {
  ...
  return {
    ...actual,
    isLibrary: vi.fn().mockResolvedValue(true),
...
describe('upgrade library package', () => {
  beforeEach(async () => {
    vi.spyOn(console, 'log').mockImplementation(() => {});
    rootDir = await mkdtemp(path.join(os.tmpdir(), 'mb-run-upgrade-library-'));
    await writeFixture(
      'package.json',
      JSON.stringify(
        {
          name: 'upgrade-library-fixture',
          version: '1.0.0',
          ...
          repository: { type: 'git', url: 'https://example.com/upgrade-library-fixture.git' },
          bugs: { url: 'https://example.com/upgrade-library-fixture/issues' },
          ...
          scripts: { start: 'node existing.js' },
          devDependencies: { jest: '0.0.0', keep: '1.0.0', typescript: '0.0.0', vitest: '0.0.0' },
        },
        null,
        2,
      ),
  ...
  it('upgrades a library package with Jest and Vitest', async () => {
    ...
    const [productionTsconfig, readme, changelog] = await Promise.all([
      readFile(path.join(rootDir, 'tsconfig.build.production.json'), 'utf8'),
    ...
    expect(packageJson.scripts).toMatchObject({
      'start': 'node existing.js',
      'test': 'cross-env NODE_OPTIONS="--experimental-vm-modules --no-warnings" jest',
      'test:vitest': 'vitest run',
    });
    expect(productionTsconfig).toContain('"declaration": true');
    expect(productionTsconfig).toContain('"removeComments": false');
