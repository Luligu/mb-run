/**
 * @file vitest/upgrade.plugin.test.ts
 * @description This file contains the tests for upgrading plugin packages.
 * @author Luca Liguori
 */
...
vi.mock('../src/helpers.js', async (importOriginal) => {
  ...
  return {
    ...
    isPlugin: vi.fn().mockResolvedValue(true),
  };
});
...
describe('upgrade plugin package', () => {
  beforeEach(async () => {
    vi.spyOn(console, 'log').mockImplementation(() => {});
    rootDir = await mkdtemp(path.join(os.tmpdir(), 'mb-run-upgrade-plugin-'));
    await writeFixture(
      'package.json',
      JSON.stringify(
        {
          name: 'upgrade-plugin-fixture',
          version: '1.0.0',
          ...
          repository: { type: 'git', url: 'https://example.com/upgrade-plugin-fixture.git' },
          bugs: { url: 'https://example.com/upgrade-plugin-fixture/issues' },
  ...
  it('upgrades a plugin package with Jest and Vitest', async () => {
    ...
    expect(packageJson.scripts).toMatchObject({
      'start': 'matterbridge',
      'add': 'matterbridge --add .',
      'remove': 'matterbridge --remove .',
      'link': 'npm link --no-fund --no-audit matterbridge',
