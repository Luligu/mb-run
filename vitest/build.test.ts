...
describe('build', () => {
  beforeEach(() => {
    ...
    // Reset call history and any queued one-time implementations so tests do not leak into each other (clearMocks is off).
  ...
  describe('runBin — known entrypoint (tsc)', () => {
    ...
    it('throws ExitError with "Missing binary" message when entrypoint is absent', async () => {
      const opts = makeOpts({ rootDir: '/nonexistent-abc-xyz', dryRun: false });
      await expect(runBin('tsc', [], opts)).rejects.toMatchObject({
        message: expect.stringContaining('Missing binary'),
      });
    });
  ...
  describe('runBin — unknown bin (shim path)', () => {
    ...
    it('uses the .cmd shim path when isWindows=true', async () => {
      ...
      try {
        await mkdir(path.join(tmpDir, 'node_modules', '.bin'), {
          recursive: true,
        });
    ...
    it('uses the plain shim path when isWindows=false', async () => {
      ...
      try {
        ...
        mockCrossSpawn.mockImplementationOnce(() => makeChild(0));
        const opts = makeOpts({ isWindows: false, dryRun: false, rootDir: tmpDir });
        await runBin('shx', [], opts);
  ...
  describe('runWorkspaceBuild', () => {
    ...
    beforeEach(async () => {
      ...
      // isMonorepo (used to pick -b vs -p) reads package.json; default to a single (non-monorepo) package.
    ...
    it('uses tsc -b (build mode) for a monorepo', async () => {
      ...
      expect(lines.some((l) => l.includes('-p '))).toBe(false);
    });

    it('uses tsc -p (project mode) for a single package', async () => {
      await writeFile(path.join(tmpDir, 'tsconfig.build.json'), '{}');
  ...
  describe('runWorkspaceTypecheck', () => {
    it('uses tsconfig.json with --noEmit', async () => {
      const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'mb-run-typecheck-'));
      try {
        await writeFile(path.join(tmpDir, 'package.json'), '{}');
        await runWorkspaceTypecheck(makeOpts({ rootDir: tmpDir }));

        expect(lines.some((line) => line.includes(path.join(tmpDir, 'tsconfig.json')))).toBe(true);
        expect(lines.some((line) => line.includes('--noEmit'))).toBe(true);
      } finally {
        await rm(tmpDir, { recursive: true, force: true });
      }
    });
