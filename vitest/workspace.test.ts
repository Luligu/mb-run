...
describe('workspace', () => {
  beforeEach(async () => {
    ...
    mockReadFile.mockImplementation(async (filePath: unknown, ...args: unknown[]) => {
      if (filePath === rootPkgPath) return JSON.stringify({ name: 'root', version: '1.0.0' });
      return actualFs.readFile(filePath as string, ...(args as [BufferEncoding]));
    });
    ...
    mockFileExists.mockImplementation(async (filePath: string) => (filePath === rootPkgPath ? true : actualClean.fileExists(filePath)));
  });
  ...
  describe('main — workspace explicit paths', () => {
    ...
    it('updates cross-workspace dependency versions in dry-run (covers logWriteFile)', async () => {
      ...
      const rootPkg = JSON.stringify({
        name: 'root',
        version: '1.0.0',
        workspaces: ['packages/ws1', 'packages/ws2'],
      });
      const ws1Pkg = JSON.stringify({ name: 'ws1', version: '1.0.0' });
      const ws2Pkg = JSON.stringify({
        name: 'ws2',
        version: '1.0.0',
        dependencies: { ws1: '1.0.0' },
      });
  ...
  describe('main — workspace object format', () => {
    it('handles { packages: [...] } format', async () => {
      ...
      const rootPkg = JSON.stringify({
        name: 'root',
        version: '1.0.0',
        workspaces: { packages: ['packages/ws1'] },
      });
      const ws1Pkg = JSON.stringify({ name: 'ws1', version: '1.0.0' });
  ...
  describe('main — cross-workspace dep edge cases', () => {
    ...
    it('does not update a dep already at the target version', async () => {
      ...
      const ws2Pkg = JSON.stringify({
        name: 'ws2',
        version: '1.0.0',
        dependencies: { ws1: targetVersion },
      });
