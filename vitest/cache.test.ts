/**
 * @file vitest/cache.test.ts
 * @description This file contains the tests for the package.json and tsconfig backup/restore utilities.
 * @author Luca Liguori
 */
...
async function createMonorepoFixture(): Promise<string> {
  const rootDir = await mkdtemp(path.join(os.tmpdir(), 'mb-run-cache-monorepo-'));
  const writeJson = async (filePath: string, content: Record<string, unknown>): Promise<void> => {
  ...
  for (const name of ['tsconfig.json', 'tsconfig.base.json', 'tsconfig.build.json', 'tsconfig.build.production.json']) {
    await writeJson(path.join(rootDir, name), { compilerOptions: { target: 'ES2022' } });
  }
  for (const packageName of ['one', 'two']) {
    const workspaceDir = path.join(rootDir, 'packages', packageName);
    await writeJson(path.join(workspaceDir, 'package.json'), { name: `@monorepo/${packageName}`, version: '1.0.0' });
    for (const name of ['tsconfig.json', 'tsconfig.build.json', 'tsconfig.build.production.json']) {
      await writeJson(path.join(workspaceDir, name), { compilerOptions: { target: 'ES2022' } });
    }
  }
  return rootDir;
}
...
describe('cache', () => {
  beforeEach(async () => {
    monorepoPath = await createMonorepoFixture();
    await backup(monorepoPath);
  });

  afterEach(async () => {
    await rm(monorepoPath, { recursive: true, force: true });
  });
    ...
    it('skips empty string patterns and resolves explicit non-glob paths', async () => {
      ...
      try {
        ...
        await writeFile(
          ...
          JSON.stringify({
            name: 'e-root',
            workspaces: ['', 'packages/one', 'packages/missing'],
          }),
        );
        await mkdir(path.join(tmpDir, 'packages', 'one'), { recursive: true });
    ...
    it('silently ignores a complex glob pattern and a glob with a missing base dir', async () => {
      ...
      try {
        ...
        await writeFile(
          ...
          JSON.stringify({
            name: 'cg-root',
            workspaces: ['complex/**', 'nonexistent/*'],
          }),
        );
        await backup(tmpDir);
