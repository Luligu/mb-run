...
vi.mock('../src/format.js', () => ({ runFormatter: vi.fn() }));
vi.mock('../src/sort.js', () => ({ sortAll: vi.fn() }));
...
describe('module', () => {
  ...
  describe('main — argument validation', () => {
    ...
    it('prints usage and throws ExitError when the current directory has no package.json', async () => {
      const emptyDir = await mkdtemp(path.join(os.tmpdir(), 'mb-run-module-empty-'));
  ...
  describe('main — --dry-run build operations', () => {
    ...
    it('--clean with ANSI enabled covers the moveUp branch of restorePos', async () => {
      const libraryDir = await mkdtemp(path.join(os.tmpdir(), 'mb-run-module-library-'));
      await writeFile(path.join(libraryDir, 'package.json'), JSON.stringify({ name: 'library', version: '1.0.0' }));
      vi.spyOn(ansiModule, 'shouldUseAnsi').mockReturnValue(true);
      vi.spyOn(process, 'cwd').mockReturnValue(libraryDir);
      try {
        setArgs('--clean');
        await expect(main()).resolves.toBeUndefined();
      } finally {
    ...
    it('--dry-run --typecheck resolves without error', async () => {
      setArgs('--dry-run', '--typecheck');
      await expect(main()).resolves.toBeUndefined();
    });
  ...
  describe('main — --dry-run quality operations', () => {
    ...
    it('--dry-run --test accepts test runner keywords', async () => {
      setArgs('--dry-run', '--test', 'verbose', 'watch', 'coverage');
      await expect(main()).resolves.toBeUndefined();
      expect(runTests).toHaveBeenLastCalledWith(expect.objectContaining({ verbose: true, watch: true, coverage: true }));
    });

    it('test runner keywords stop at the next flag', async () => {
      setArgs('--dry-run', '--test', 'verbose', '--build');
      await expect(main()).resolves.toBeUndefined();
      expect(runTests).toHaveBeenLastCalledWith(expect.objectContaining({ verbose: true, watch: false, coverage: false }));
    });

    it('unrecognized token after --test throws ExitError', async () => {
      setArgs('--dry-run', '--test', 'unknown');
      await expect(main()).rejects.toBeInstanceOf(ExitError);
    });
    ...
    it('--dry-run --update links matterbridge when the current package is a plugin', async () => {
      const pluginDir = await mkdtemp(path.join(os.tmpdir(), 'mb-run-module-update-plugin-'));
      await writeFile(path.join(pluginDir, 'package.json'), JSON.stringify({ name: 'plugin', version: '1.0.0', scripts: { start: 'matterbridge' } }));
  ...
  describe('main — --dry-run install operations', () => {
    ...
    it('--dry-run --install links matterbridge when the current package is a plugin', async () => {
      const pluginDir = await mkdtemp(path.join(os.tmpdir(), 'mb-run-module-install-plugin-'));
      await writeFile(path.join(pluginDir, 'package.json'), JSON.stringify({ name: 'plugin', version: '1.0.0', scripts: { start: 'matterbridge' } }));
      vi.spyOn(process, 'cwd').mockReturnValue(pluginDir);
      try {
        setArgs('--dry-run', '--install');
  ...
  describe('main — --dry-run clean operations', () => {
    ...
    it('--dry-run --reset links matterbridge when the current package is a plugin', async () => {
      const pluginDir = await mkdtemp(path.join(os.tmpdir(), 'mb-run-module-reset-plugin-'));
      await writeFile(path.join(pluginDir, 'package.json'), JSON.stringify({ name: 'plugin', version: '1.0.0', scripts: { start: 'matterbridge' } }));
      vi.spyOn(process, 'cwd').mockReturnValue(pluginDir);
      try {
        setArgs('--dry-run', '--reset');
  ...
  describe('main — --sort', () => {
    ...
    it('sorts package.json files when not running dry', async () => {
      const packageDir = await mkdtemp(path.join(os.tmpdir(), 'mb-run-module-sort-'));
      await writeFile(path.join(packageDir, 'package.json'), JSON.stringify({ name: 'sort-test', version: '1.0.0' }));
      vi.spyOn(process, 'cwd').mockReturnValue(packageDir);
      try {
        setArgs('--sort');
        await expect(main()).resolves.toBeUndefined();
        expect(vi.mocked(sortAll)).toHaveBeenCalledWith(packageDir);
        expect(vi.mocked(runFormatter)).toHaveBeenCalled();
      } finally {
  ...
  describe('main — --esbuild', () => {
    ...
    it('passes the minify keyword to esbuild', async () => {
      setArgs('--dry-run', '--esbuild', 'minify');
      await expect(main()).resolves.toBeUndefined();
      expect(runEsbuild).toHaveBeenLastCalledWith(expect.objectContaining({ minify: true }));
    });

    it('stops reading esbuild keywords at the next flag', async () => {
      setArgs('--dry-run', '--esbuild', 'minify', '--build');
      await expect(main()).resolves.toBeUndefined();
      expect(runEsbuild).toHaveBeenLastCalledWith(expect.objectContaining({ minify: true }));
    });

    it('rejects unknown keywords after --esbuild', async () => {
      setArgs('--dry-run', '--esbuild', 'unknown');
      await expect(main()).rejects.toBeInstanceOf(ExitError);
    });
  });

  describe('main — --pack', () => {
    it('passes minify to pack only when requested', async () => {
      setArgs('--dry-run', '--pack', 'edge', 'minify');
      await expect(main()).resolves.toBeUndefined();
      expect(runPack).toHaveBeenLastCalledWith(expect.objectContaining({ minify: true, tag: 'edge' }));
    });

    it('does not minify when the keyword is omitted', async () => {
      setArgs('--dry-run', '--pack', 'edge');
      await expect(main()).resolves.toBeUndefined();
      expect(runPack).toHaveBeenLastCalledWith(expect.objectContaining({ minify: false, tag: 'edge' }));
    });

    it('stops reading pack keywords at the next flag', async () => {
      setArgs('--dry-run', '--pack', 'minify', '--build');
      await expect(main()).resolves.toBeUndefined();
      expect(runPack).toHaveBeenLastCalledWith(expect.objectContaining({ minify: true, tag: null }));
    });

    it('rejects a second positional argument after the pack tag', async () => {
      setArgs('--dry-run', '--pack', 'edge', 'unexpected');
      await expect(main()).rejects.toBeInstanceOf(ExitError);
    });
  });
  ...
  describe('main — --upgrade', () => {
    ...
    it('--dry-run --upgrade runs npm link matterbridge when cwd is a plugin', async () => {
      const pluginDir = await mkdtemp(path.join(os.tmpdir(), 'mb-run-module-plugin-'));
      await writeFile(path.join(pluginDir, 'package.json'), JSON.stringify({ name: 'plugin', version: '1.0.0', scripts: { start: 'matterbridge' } }));
      vi.spyOn(process, 'cwd').mockReturnValue(pluginDir);
      try {
        setArgs('--dry-run', '--upgrade');
