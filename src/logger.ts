...
export class Logger {
  ...
  /**
   * Logs an esbuild configuration action when verbose or dry-run mode is active.
   *
   * @param {string} action Esbuild action name.
   * @param {string[]} args Action arguments.
   * @param {string} [cwd] Working directory; defaults to the logger's rootDir.
   * @returns {void}
   */
  logEsbuildAction = (action: string, args: string[], cwd: string = this.rootDir): void => {
    if (!this.shouldLogActions()) return;
    log(`${this.logPrefix()} ${cyan(cwd)}> ${magenta('esbuild')} ${action} ${args.map(formatCommandArg).join(' ')}`);
  };

  /**
   * Logs formatted esbuild options when verbose or dry-run mode is active.
   *
   * @param {string} options Formatted esbuild options.
   * @param {string} [cwd] Working directory; defaults to the logger's rootDir.
   * @returns {void}
   */
  logEsbuildOptions = (options: string, cwd: string = this.rootDir): void => {
    if (!this.shouldLogActions()) return;
    log(`${this.logPrefix()} ${cyan(cwd)}> ${magenta('esbuild')} options:\n${options}`);
  };
  ...
  /**
   * Logs a warning diagnostic (always shown, regardless of verbose/dry-run state).
   *
 ...
 * @returns {void}
 */
export function logEsbuildAction(action: string, args: string[], cwd?: string): void {
  logger.logEsbuildAction(action, args, cwd);
}
 ...
 * @returns {void}
 */
export function logEsbuildOptions(options: string, cwd?: string): void {
  logger.logEsbuildOptions(options, cwd);
}
