 ...
 * @param {Pick<BuildOptions, 'rootDir' | 'isWindows'>} opts Build options.
 * @returns {string} Absolute path to the shim.
 */
function binPath(binName: string, opts: Pick<BuildOptions, 'rootDir' | 'isWindows'>): string {
 ...
 */
function entrypointPath(binName: string, opts: Pick<BuildOptions, 'rootDir' | 'isWindows'>): string | null {
...
/**
 * Reports whether a tool is installed, without throwing.
 *
 * @param {string} binName The tool name.
 * @param {Pick<BuildOptions, 'rootDir' | 'isWindows'>} opts Build options.
 * @returns {Promise<boolean>} True when the tool's binary or entrypoint exists.
 */
export async function binExists(binName: string, opts: Pick<BuildOptions, 'rootDir' | 'isWindows'>): Promise<boolean> {
  const entry = entrypointPath(binName, opts);
  const p = entry ?? binPath(binName, opts);
  try {
    await access(p);
