...
import { runWorkspaceBuild, runWorkspaceTypecheck } from './build.js';
...
export async function main(): Promise<void> {
  ...
  const packArgIndices = new Set<number>();
  let rawPackTag: string | undefined;
  let packMinify = false;
  if (packIndex >= 0) {
    for (let i = packIndex + 1; i < rawArgs.length; i++) {
      const packArg = rawArgs[i];
      if (packArg.startsWith('-')) break;
      if (packArg === 'minify') {
        packMinify = true;
        packArgIndices.add(i);
      } else if (rawPackTag === undefined) {
        rawPackTag = packArg;
        packArgIndices.add(i);
      }
    }
  ...
  const testIndex = rawArgs.indexOf('--test');
  const testValidKeywords = new Set(['verbose', 'watch', 'coverage']);
  const testArgIndices = new Set<number>();
  const testArgs = new Set<string>();
  if (testIndex >= 0) {
    for (let i = testIndex + 1; i < rawArgs.length; i++) {
      if (rawArgs[i].startsWith('-')) break;
      if (testValidKeywords.has(rawArgs[i])) {
        testArgs.add(rawArgs[i]);
        testArgIndices.add(i);
      }
    }
  }

  const esbuildIndex = rawArgs.indexOf('--esbuild');
  const esbuildValidKeywords = new Set(['minify']);
  const esbuildArgIndices = new Set<number>();
  const esbuildArgs = new Set<string>();
  if (esbuildIndex >= 0) {
    for (let i = esbuildIndex + 1; i < rawArgs.length; i++) {
      if (rawArgs[i].startsWith('-')) break;
      if (esbuildValidKeywords.has(rawArgs[i])) {
        esbuildArgs.add(rawArgs[i]);
        esbuildArgIndices.add(i);
      }
    }
  }
  ...
  if (want.reset) {
    ...
    await runWorkspaceBuild({
      ...buildOpts,
      mode: want.production ? 'production' : 'build',
      watch: false,
    });
  ...
  if (want.typecheck) {
    log(`${savePos()}⏳ Type-checking...`);
    await runWorkspaceTypecheck({ ...buildOpts, mode: 'build', watch: false });
    log(`${restorePos()}${green('✅')} Type-check complete in ${getElapsed()}.${clearEnd()}`);
  }
  ...
  if (want.test) {
    ...
    await runTests({
      ...buildOpts,
      verbose: testArgs.has('verbose'),
      watch: testArgs.has('watch'),
      coverage: testArgs.has('coverage'),
    });
    log(`${restorePos()}${green('✅')} Tests complete in ${getElapsed()}.${clearEnd()}`);
  }

  if (want.format) {
    log(`${savePos()}⏳ ${want.check ? 'Checking format' : 'Formatting'}...`);
    await runFormatter({ ...buildOpts, check: want.check });
    log(`${restorePos()}${green('✅')} Format ${want.check ? 'check ' : ''}complete in ${getElapsed()}.${clearEnd()}`);
  }

  if (want.lint) {
    log(`${savePos()}⏳ ${want.fix ? 'Linting with fix' : 'Linting'}...`);
    await runLinter({ ...buildOpts, fix: want.fix });
    log(`${restorePos()}${green('✅')} Lint ${want.fix ? 'fix ' : ''}complete in ${getElapsed()}.${clearEnd()}`);
  }
  ...
  if (want.sort) {
    ...
    await runFormatter({ ...buildOpts, check: false });
    log(`${restorePos()}${green('✅')} Sort complete in ${getElapsed()}.${clearEnd()}`);
  }

  if (want.pack) {
    log(`${savePos()}⏳ Packing...`);
    await runPack({ ...buildOpts, tag: packTag, minify: packMinify });
  ...
  if (want.esbuild) {
    ...
    await runEsbuild({ ...buildOpts, minify: esbuildArgs.has('minify') });
