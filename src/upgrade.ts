...
export async function runPackageJsonUpgrade(
  ...
  if (updateScript) {
    ...
    pkgJson.scripts = {
      ...
      'buildDts': isLibrary ? 'tsc --project tsconfig.build.json --incremental false --composite false --declaration --declarationMap false --emitDeclarationOnly' : undefined,
      'buildProduction': 'tsc --project tsconfig.build.production.json',
      'clean': 'node scripts/clean.mjs',
      'cleanBuild': 'npm run clean && npm run build',
      'cleanBuildDts': isLibrary ? 'npm run clean && npm run buildDts' : undefined,
      ...
      'git:merge:dev':
        automator?.git === true ? 'git fetch origin && git checkout -b dev-backup-001 && git checkout dev && git merge origin/main && git push origin dev' : undefined,
      'git:rebase:dev':
        automator?.git === true
